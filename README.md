...
Kaggle 上の DICOM データセットから自分たちで `.npy` に変換する作業にかなり手間取った。元データが 200 GB 超と巨大なうえ、DICOM のメタデータ（Slice Thickness、Pixel Spacing、Rescale Slope/Intercept など）を正しくハンドリングしながらボリュームを再構成し、適切な Window レベルで正規化するという一連の処理に経験不足を痛感した。この前処理パイプラインをより効率的かつ堅牢に構築できるようになることが重要な課題。
